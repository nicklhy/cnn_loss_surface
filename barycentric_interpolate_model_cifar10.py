...
if __name__ == '__main__':
        ...
        for j in range(alpha_grid.shape[1]):
            ...
            for name in tmp_params.keys():
                for k, theta_0 in params1[name].items():
                    ...
                    tmp_params[name][k] = beta*(alpha*(theta_1-theta_0)+theta_0)+(1-beta)*(alpha*(theta_2-theta_0)+theta_0)
    ...
    if args.output is None:
        args.output = os.path.join(ROOT_DIR,
                                   'cache',
                                   os.path.split(args.params1)[-1].replace('.params', '')+'_'+os.path.split(args.params2)[-1].replace('.params', '')+'_'+os.path.split(args.params3)[-1].replace('.params', '')+'_barycentric_interpolate.pkl')

    ...
    for alpha in alpha_list:
        ...
        for name in tmp_params.keys():
            for k, theta_0 in params1[name].items():
                ...
                tmp_params[name][k] = theta_0*(1-alpha)+theta_1*alpha
        ...
        if train is not None:
            ...
            train_error_list.append(1-eval_metrics.get()[1])
            print '\ttrain error = %f' % train_error_list[-1]
        if val is not None:
            model.score(val, eval_metrics, reset=True)
            val_error_list.append(1-eval_metrics.get()[1])
            print '\tval error = %f' % val_error_list[-1]

    if args.output is None:
        args.output = os.path.join(ROOT_DIR,
                                   'cache',
                                   args.dataset,
                                   os.path.split(args.params1)[-1].replace('.params', '')+'_'+os.path.split(args.params2)[-1].replace('.params', '')+'_linear_interpolate.pkl')
